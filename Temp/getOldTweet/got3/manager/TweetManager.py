	...
	def getTweets(tweetCriteria, receiveBuffer = None, bufferLength = 100):
		...
		while active:
			...
			for tweetHTML in tweets:
				...
				temp_txt= PyQuery(re.sub(r'</span><span class="js-display-url">|</span><span class="invisible">', '', str(tweetPQ("p.js-tweet-text")))).text()
				txt = re.sub(r"\s+", " ", temp_txt.replace('# ', '#').replace('@ ', '@').replace('…', '...'))
				...
				tweet.geo = geo
				tweet.urls = urls
				tweet.pics = re.compile('(pic.twitter.com/\\S+)').findall(tweet.text)
